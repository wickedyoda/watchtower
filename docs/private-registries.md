...
```bash
docker run [...] -v <PATH>/config.json:/config.json ghcr.io/wickedyoda/watchtower:latest
```
...
```bash
docker run [...] -v <PATH_TO_HOME_DIR>/.docker/config.json:/config.json ghcr.io/wickedyoda/watchtower:latest
```
...
services:
  watchtower:
    image: ghcr.io/wickedyoda/watchtower:latest:latest
    volumes:
