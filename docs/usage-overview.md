Watchtower is itself packaged as a Docker container so installation is as simple as pulling the `ghcr.io/wickedyoda/watchtower:latest` image. If you are using ARM based architecture, pull the appropriate `ghcr.io/wickedyoda/watchtower:arm64v8-latest` image from the [containrrr Docker Hub](https://hub.docker.com/r/containrrr/watchtower/tags/).
...
docker run -d \
  ...
  -v /var/run/docker.sock:/var/run/docker.sock \
  ghcr.io/wickedyoda/watchtower:latest container_to_watch --debug
```
