...
It reacts to Docker lifecycle events when possible and falls back to polling so the sync keeps working if the event stream drops.
...
1. Edit `config.yml`:
   ...
   - Optional: adjust `bot.request_timeout_seconds` if your Docker API is slow to answer
...
- The event listener watches mapped Docker hosts for container `create`, `start`, `restart`, and `rename` events and wakes the sync loop immediately.
